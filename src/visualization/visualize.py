...
from src.utils.imports import load_visualization_config
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        labels = pd.read_csv(
            os.path.join(Path(config.label_folder), f"{file}.csv")
        )
        ...
        # plot predictions is predictions are provided
        if config.predictions_folder:
            # plot predictions
            predictions = pd.read_csv(
                os.path.join(Path(config.predictions_folder), f"{file}.csv")
            )
